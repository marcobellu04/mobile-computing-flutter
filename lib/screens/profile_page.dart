...
import '../providers/booking_provider.dart'; // Assicurati sia corretto
...
class _ProfilePageState extends State<ProfilePage> {
  ...
  void _showManagementArea(BuildContext context, List<Event> events, List<Venue> venues) {
    ...
    // Filtriamo le strutture che hanno almeno una richiesta 'pending'
 ...
 void _showCreatedEventsList(BuildContext context, List<Event> events, {bool onlyWithRequests = false}) {
    final filtered = onlyWithRequests
        ? events.where((e) => e.pendingRequests.isNotEmpty).toList()
        : events;
                    ...
                    itemBuilder: (ctx, i) {
                      ...
                      return ListTile(
                        ...
                        onTap: () {
                          ...
                          // MODIFICA QUI:
                          if (onlyWithRequests) {
                            // Se arriviamo dall'Area Gestione, andiamo DIRETTI alle richieste
                            Navigator.push(
                              context, 
                              MaterialPageRoute(builder: (_) => EventRequestsPage(eventId: e.id, eventName: e.name))
                            );
                          } else {
                            // Altrimenti andiamo al dettaglio normale
                            Navigator.push(
                              context, 
  ...
  void _showCreatedVenuesList(BuildContext context, List<Venue> venues, {bool isManagementMode = false}) {
    ...
    final filtered = isManagementMode

        ? venues.where((v) => bookingProvider.requests.any((r) => r.venueEmail == v.ownerEmail && r.status == 'pending')).toList()

        : venues;



    // RIMOSSO: il controllo if (filtered.isEmpty) con lo SnackBar
      ...
      builder: (context) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(isManagementMode ? 'STRUTTURE CON RICHIESTE' : 'LE MIE STRUTTURE',
            ...
            Expanded(
              ...
              child: filtered.isEmpty
                ...
                ? const Center(child: Text("Nessuna struttura presente", style: TextStyle(color: Colors.grey))) // Visualizza testo vuoto invece di SnackBar
          ...
          children: [
            ...
            _buildSectionCard([
              ...
              ListTile(
                ...
                title: const Text('Elimina Account', style: TextStyle(color: Colors.grey)),
