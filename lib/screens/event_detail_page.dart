...
if (isOwner && current.listType == ListType.closed) ...[
  ...
  ListTile(
    ...
    title: const Text("Gestisci Richieste", style: TextStyle(fontWeight: FontWeight.bold)),
    subtitle: Text("${current.pendingRequests.length} persone in attesa"),
    trailing: const Icon(Icons.arrow_forward_ios, size: 16),
    onTap: () {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (_) => EventRequestsPage(
            eventId: current.id,
